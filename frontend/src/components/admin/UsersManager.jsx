...
export default function UsersManager() {
  ...
  const changeRole = async (u, nextRole) => {
    ...
    if (!confirm(`เปลี่ยนบทบาทของ ${u.username || u.email} เป็น "${nextRole}" ?`)) return;
    setRowBusy(id, true);
    try {
  ...
  const fmtDate = (s) => {
    try {
      return new Date(s).toLocaleString("th-TH", { year: "numeric", month: "short", day: "numeric" });
    } catch { return "—"; }
  };
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-gray-100">ผู้ใช้งาน</h2>
          <p className="text-gray-600 dark:text-gray-400">ค้นหา กรอง ปรับบทบาท และจัดการสถานะผู้ใช้งาน</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:auto-cols-max gap-2 w-full md:w-auto">
          <div className="inline-flex items-center px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-800 w-full">
            ...
            <input
              ...
              className="bg-transparent outline-none text-sm w-full min-w-0 placeholder:text-gray-400 dark:text-gray-100"
            />
          </div>
          ...
          <select
            ...
            className="px-3 py-2 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-800 text-sm w-full"
          >
      ...
      <div className="p-4 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-800">
        {loading ? (
          ...
          <div className="py-8 text-center text-gray-500 dark:text-gray-400">ไม่พบผู้ใช้งาน</div>
        ) : (
          <>
            {/* Card view (mobile) */}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className={`text-xs px-2 py-0.5 rounded-full ${u.status === "active"
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <select
                            ...
                            className="text-xs rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-800"
                          >
                            <option value="user">user</option>
                            <option value="owner">owner</option>
                            <option value="admin">admin</option>
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="text-left text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-white/10">
                    ...
                    <th className="py-2 pr-4 hidden lg:table-cell">อีเมล</th>
                    <th className="py-2 pr-4">บทบาท</th>
                    <th className="py-2 pr-4">สถานะ</th>
                    <th className="py-2 pr-4 hidden lg:table-cell">ยืนยัน</th>
                    <th className="py-2 pr-4 hidden xl:table-cell">สมัครเมื่อ</th>
                  ...
                  {items.map((u) => {
                    ...
                    return (
                      <tr key={id} className="border-b last:border-0 border-gray-100 dark:border-white/5">
                        ...
                        <td className="py-2 pr-4 text-gray-700 dark:text-gray-300 hidden lg:table-cell">{u.email || "—"}</td>
